...
class Game:
	...
	def __init__(self,path):
		...
		self.cards = self.gameJson["cards"] #TODO: card action olmasi lazim
		self.currRound = 0
		self.cells = []
		self.players = []
		#self.lock = th.Lock()
...
########################################################################################################
	...
	def _takeAction(self, action, player):
		...
		if(actionKey == "skip"):
			...
			actionChange.append({"skip": actionValue})
		
		elif(actionKey == "drop"):
			player.credit -= actionValue # check
			actionChange.append({"drop": actionValue})
			...
			if(isinstance(actionValue,str)): # Absolute Jump
				...
				actionChange.append({"jump absolute =": actionValue,"current cell":player.cellNo})
			else:
				player.cellNo += actionValue
				if player.cellNo >= self.cellCount:
					if self.cycles == True:
						...
						self.gameState = 1
				actionChange.append({"jump relative": actionValue,"current cell":player.cellNo})
...
########################################################################################################
	...
	def next(self, player):
		...
		""" elif(player.currType == "artifact"): # Burada olmayacak bu, pick de olacak
			pass """
